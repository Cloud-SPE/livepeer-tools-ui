import type { DateRange, PayoutLeaderboardRow, PeriodKind } from "./types";

/* ---------- date helpers ---------- */

/**
 * Parse an ISO YYYY-MM-DD date string into a UTC Date. Returns null on
 * invalid input. We treat dates as UTC midnight throughout the domain so
 * day-boundary arithmetic is timezone-stable.
 */
...
/**
 * Monday of the week containing `d` (UTC). ISO weeks start on Monday.
 */
function startOfIsoWeek(d: Date): Date {
  // getUTCDay: Sun=0, Mon=1, ..., Sat=6. We want offset to Monday (-0..-6).
...
/**
 * Date range covered by a period containing `iso`. Both endpoints are ISO
 * YYYY-MM-DD; `from` is inclusive, `to` is exclusive (next-day for daily,
 * next Monday for weekly, first-of-next-month for monthly).
 */
...
/** ISO date one period before/after `iso` for prev/next navigation. */
...
/** Display label fallback when a leaderboard row has no display name. */
export function rowLabel(row: PayoutLeaderboardRow): string {
  const n = row.displayName?.trim();
  return n ? n : shortAddress(row.orchestratorAddress);
}

/** Format a date for human display (e.g. "May 14, 2026"). */
